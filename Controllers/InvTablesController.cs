        ...
        public ActionResult SelectInvDt(string Letter, int Num )  //todo 是否改吃 LetterId(?)
        {
            if (Session["Guid"] != null)
            {
                ...
                if (invAccount != null)
                {
                    ...
                    DateTime LowerLimit = Convert.ToDateTime(invTable.Where(T => Convert.ToInt32(T.Num) < Num).Select(T=>T.InvDate).LastOrDefault());

                    DateTime UpperLimit = Convert.ToDateTime(invTable.Where(T => Convert.ToInt32(T.Num) > Num).Select(T => T.InvDate).LastOrDefault());
            ...
            if (Session["Guid"] != null)
            {
                ...
                if (Term != "" && (StarTime != null && EndTime != null))
                {
                    ...
                    var invTable = db.InvTables.Where(T => T.AccountId == invAccount.Id &&
                                                           ...
                                                           T.InvDate < NewEndTime &&
                                                           T.Client.Contains(Term)
                        ).Select(T => new { T.Id, T.InvDate, T.InvNum, T.Client, T.Total, T.InvStatus,T.Letter }).ToList();


                    var invTablePage = invTable.OrderByDescending(T => T.InvDate).ToPagedList(Page, DefaultPageSize);
        ...
        public ActionResult LoadInvEd(int Id)
        {
            if (Session["Guid"] != null)
            {
                ...
                if (ModelState.IsValid)
                {
                    var invTable = db.InvTables.Where(T => T.Id == Id && T.AccountId == invAccount.Id && T.InvStatus == InvStatusType.已開立 ).Select(T => new
                    {
                        T.InvAccount.UniformNumbers, T.InvAccount.CompName, T.InvAccount.CompPhoneNumber, T.InvAccount.CompAddress,
                        T.Client, T.ClientUniformNum, T.ClientPhoneNumber, T.ClientAdress,
                        T.Letter, T.Num, T.InvDate,T.Id,
                        T.InvItems
                    }).FirstOrDefault();
